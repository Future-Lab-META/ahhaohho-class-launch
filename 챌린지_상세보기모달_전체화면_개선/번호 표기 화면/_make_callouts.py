...
FRAMES = [
    dict(
        # 공통 1~8 전용 — 화면별 캡쳐에는 그 화면의 번호만 남김
        name='공통',
        ...
        co={
            ...
            '공통 8': {'sel': '.navArrow.next', 'at': 'bottom', 'dy': 40, 'label': '좌우 화면 이동', 'tone': 'green'},
        },
    ),
    dict(
        name='개요',
        setup="setTab('view'); show(0); pickGo(1);",
        co={
            '1':   {'sel': '#s0Slides', 'at': 'left', 'dx': -76, 'dy': 120, 'label': '썸네일·Pick 이미지 영역'},
            '2-1': {'sel': '#pickNext', 'at': 'right', 'dx': 60, 'label': '슬라이드 이동 화살표', 'tone': 'green'},
            '2-2': {'sel': '#pickDots', 'at': 'left', 'dx': -64, 'label': '인디케이터', 'tone': 'green'},
            '2-3': {'sel': '#s0 .pickTag', 'at': 'left', 'dx': -76, 'label': '프로젝트 게시자', 'tone': 'green'},
